...
def check_winner():        
    ...
    for row in range(3):
        if (board[row][0]["text"] == board[row][1]["text"] == board[row][2]["text"]
            ...
            for column in range(3):
              board[row][column].config(foreground=color_orange, background=color_light_gray)
            game_over = True, label.config(text=board[0][0]["text"]+" is winner!", foreground=color_orange)
            return

    #vertically, check 3 clolumns
    for column in range(3):
   ...
   #diagonally
    if (board[0][0]["text"] == board[1][1]["text"] == board[2][2]["text"]
       and board[0][0]["text"] != ""):
       label.config(text=board[0][0]["text"]+" is winner!", foreground=color_orange)
       for i in range(3):
           board[i][i].config(foreground=color_orange, background=color_light_gray)
       game_over = True
       return 

   #anti-diagionally           
    if (board[0][2]["text"] == board[2][2]["text"]
       and board[0][2]["text"] != ""):
       label.config(text=board[0][2]["text"]+" is winner!", foreground=color_orange) 
       board[0][2].config(foreground=color_orange, background=color_light_gray)
       board[0][2].config(foreground=color_orange, background=color_light_gray)
       board[0][2].config(foreground=color_orange, background=color_light_gray)
       game_over = True
       return
   ...
   #tie
    if (turns == 9):
       game_over= True
       label.config(text="Tie!", foreground=color_orange)     
